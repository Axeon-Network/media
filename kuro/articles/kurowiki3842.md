---
title: KuroWiki build 3842
permalink: KuroWiki_build_3842
aliases:
    - kurowiki 3842
---
{% include infoboxes/build.html
family='[KuroWiki](KuroWiki)'
buildtag='6.0.3842.nekori64.251022-2057'
version='6.0'
build='3842'
lab='nekori64'
compiled='2025-10-22 20:57 UTC-6'
image='kurowiki3842/homedark.png'
image_caption='Home page (dark mode)'
image2='kurowiki3842/homelight.png'
image2_caption='Home page (light mode)'
%}

**KuroWiki build 3842** is a Beta 6 build of [KuroWiki](KuroWiki), compiled on 22 October 2025 at 8:57PM UTC-6.

It retired the background used since [Build 3615](KuroWiki_build_3615) in place of two separate backgrounds for Dark and Light mode respectively.

# Gallery

<gallery>

    {% include items/gallery.html 
        image_src="kurowiki3842/settings.png" 
        alt_text="" 
        caption="Settings page"
        style="width:50%;" %}

    {% include items/gallery.html 
        image_src="kurowiki3842/article.png" 
        alt_text="" 
        caption="Article view"
        style="width:50%;" %}


    {% include items/gallery.html 
        image_src="kurowiki3842/settingslight.png" 
        alt_text="Settings with light mode enabled" 
        caption="Settings with light mode enabled"
        style="width:50%;" %}
</div>